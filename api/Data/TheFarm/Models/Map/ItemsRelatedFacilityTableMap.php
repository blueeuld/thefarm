...
use TheFarm\Models\ItemsRelatedFacility;
use TheFarm\Models\ItemsRelatedFacilityQuery;
 ...
 *
 */
class ItemsRelatedFacilityTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'TheFarm.Models.Map.ItemsRelatedFacilityTableMap';
     ...
     */
    const OM_CLASS = '\\TheFarm\\Models\\ItemsRelatedFacility';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'TheFarm.Models.ItemsRelatedFacility';
    ...
    protected static $fieldNames = array (
        ...
        self::TYPE_COLNAME       => array(ItemsRelatedFacilityTableMap::COL_ITEM_ID, ItemsRelatedFacilityTableMap::COL_FACILITY_ID, ),
    ...
    protected static $fieldKeys = array (
        ...
        self::TYPE_COLNAME       => array(ItemsRelatedFacilityTableMap::COL_ITEM_ID => 0, ItemsRelatedFacilityTableMap::COL_FACILITY_ID => 1, ),
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('ItemsRelatedFacility');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\TheFarm\\Models\\ItemsRelatedFacility');
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \TheFarm\Models\ItemsRelatedFacility $obj A \TheFarm\Models\ItemsRelatedFacility object.
     ...
     *
     * @param mixed $value A \TheFarm\Models\ItemsRelatedFacility object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \TheFarm\Models\ItemsRelatedFacility) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \TheFarm\Models\ItemsRelatedFacility object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? ItemsRelatedFacilityTableMap::CLASS_DEFAULT : ItemsRelatedFacilityTableMap::OM_CLASS;
    }
     ...
     * @return array           (ItemsRelatedFacility object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = ItemsRelatedFacilityTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = ItemsRelatedFacilityTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + ItemsRelatedFacilityTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = ItemsRelatedFacilityTableMap::OM_CLASS;
            /** @var ItemsRelatedFacility $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            ItemsRelatedFacilityTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = ItemsRelatedFacilityTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = ItemsRelatedFacilityTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(ItemsRelatedFacilityTableMap::COL_ITEM_ID);
            $criteria->addSelectColumn(ItemsRelatedFacilityTableMap::COL_FACILITY_ID);
        } else {
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(ItemsRelatedFacilityTableMap::DATABASE_NAME)->getTable(ItemsRelatedFacilityTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(ItemsRelatedFacilityTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(ItemsRelatedFacilityTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new ItemsRelatedFacilityTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a ItemsRelatedFacility or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or ItemsRelatedFacility object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \TheFarm\Models\ItemsRelatedFacility) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(ItemsRelatedFacilityTableMap::DATABASE_NAME);
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(ItemsRelatedFacilityTableMap::COL_ITEM_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(ItemsRelatedFacilityTableMap::COL_FACILITY_ID, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = ItemsRelatedFacilityQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            ItemsRelatedFacilityTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                ItemsRelatedFacilityTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return ItemsRelatedFacilityQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a ItemsRelatedFacility or Criteria object.
     *
     * @param mixed               $criteria Criteria or ItemsRelatedFacility object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from ItemsRelatedFacility object
        }
...
} // ItemsRelatedFacilityTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
ItemsRelatedFacilityTableMap::buildTableMap();
