...
use TheFarm\Models\FormFields;
use TheFarm\Models\FormFieldsQuery;
 ...
 *
 */
class FormFieldsTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'TheFarm.Models.Map.FormFieldsTableMap';
     ...
     */
    const OM_CLASS = '\\TheFarm\\Models\\FormFields';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'TheFarm.Models.FormFields';
    ...
    protected static $fieldNames = array (
        ...
        self::TYPE_COLNAME       => array(FormFieldsTableMap::COL_FIELD_ID, FormFieldsTableMap::COL_FORM_ID, FormFieldsTableMap::COL_GUEST_ONLY, ),
    ...
    protected static $fieldKeys = array (
        ...
        self::TYPE_COLNAME       => array(FormFieldsTableMap::COL_FIELD_ID => 0, FormFieldsTableMap::COL_FORM_ID => 1, FormFieldsTableMap::COL_GUEST_ONLY => 2, ),
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('FormFields');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\TheFarm\\Models\\FormFields');
     ...
     * and findPk*() calls.
     *
     * @param \TheFarm\Models\FormFields $obj A \TheFarm\Models\FormFields object.
     ...
     *
     * @param mixed $value A \TheFarm\Models\FormFields object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \TheFarm\Models\FormFields) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \TheFarm\Models\FormFields object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? FormFieldsTableMap::CLASS_DEFAULT : FormFieldsTableMap::OM_CLASS;
    }
     ...
     * @return array           (FormFields object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = FormFieldsTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = FormFieldsTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + FormFieldsTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = FormFieldsTableMap::OM_CLASS;
            /** @var FormFields $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            FormFieldsTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = FormFieldsTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = FormFieldsTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(FormFieldsTableMap::COL_FIELD_ID);
            $criteria->addSelectColumn(FormFieldsTableMap::COL_FORM_ID);
            $criteria->addSelectColumn(FormFieldsTableMap::COL_GUEST_ONLY);
        } else {
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(FormFieldsTableMap::DATABASE_NAME)->getTable(FormFieldsTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(FormFieldsTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(FormFieldsTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new FormFieldsTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a FormFields or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or FormFields object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \TheFarm\Models\FormFields) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(FormFieldsTableMap::DATABASE_NAME);
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(FormFieldsTableMap::COL_FIELD_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(FormFieldsTableMap::COL_FORM_ID, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = FormFieldsQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            FormFieldsTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                FormFieldsTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return FormFieldsQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a FormFields or Criteria object.
     *
     * @param mixed               $criteria Criteria or FormFields object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from FormFields object
        }
...
//
FormFieldsTableMap::buildTableMap();
