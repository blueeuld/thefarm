...
if ($bookingData) {
    ...
    $restrictions = $bookingData['Restrictions'];
    $personalized = $bookingData['Personalized'];
    $status = $bookingData['Status'];
    $package_id = $bookingData['PackageId'];
    $start_date = $bookingData['StartDate'];
    $end_date = $bookingData['EndDate'];
    $contact_id = $bookingData['GuestId'];
    $attachments = $bookingData['Attachments'];
    $forms = $bookingData['Forms'];
    $booking_items = $bookingData['Items'];
...
$roomsArr = $productApi->search_products(8);
$facilities = [];
foreach ($roomsArr as $item) {
    $facilities[$item['ItemId']] = $item['Title'];
}
...
$services = $this->db->get('items')->result_array();
				...
				<div class="panel">
					...
					<div class="panel-body">
						...
						<table class="table table-bordered table-striped item-packages">
							...
							<tbody>
							<?php foreach ($booking_items as $i => $booking_item) : ?>
                                <?php
								...
								<tr>
									<td>
										...
										<input <?php echo !current_user_can('CanManageGuestBookings') ? 'disabled' : '';?>
											name="package_items[<?php echo $i; ?>][item_id]"
											type="hidden" value="<?php echo $item['ItemId']; ?>">
									</td>
									<td><input <?php echo !current_user_can('CanManageGuestBookings') ? 'disabled' : '';?>
											name="package_items[<?php echo $i; ?>][qty]"
											type="text" class="form-control text-center package-items-qty" 
											data-original_qty="<?php echo $booking_item['Quantity']; ?>"
											value="<?php echo $booking_item['Quantity']; ?>">
									</td>
									<td class="text-center">
										<div class="checkbox checkbox-success checkbox-inline">
											<input <?php echo !current_user_can('CanManageGuestBookings') ? 'disabled' : '';?> id="package_items_included_<?php echo $i; ?>" name="package_items[<?php echo $i; ?>][included]" type="checkbox" <?php if ($booking_item['Included']) : ?>checked="checked"<?php endif;?> value="1">
										</div>
									</td>
									<td class="text-center">
										<div class="checkbox checkbox-success checkbox-inline">
											<input <?php echo !current_user_can('CanManageGuestBookings') ? 'disabled' : '';?> id="package_items_foc_<?php echo $i; ?>" name="package_items[<?php echo $i; ?>][foc]" type="checkbox" <?php if ($booking_item['Foc']) : ?>checked="checked"<?php endif;?> value="1">
										</div>
									</td>
									<td class="text-center">
										<div class="checkbox checkbox-success checkbox-inline">
											<input <?php echo !current_user_can('CanManageGuestBookings') ? 'disabled' : '';?> id="package_items_upsell_<?php echo $i; ?>" name="package_items[<?php echo $i; ?>][upsell]" type="checkbox" <?php if ($booking_item['Upsell']) : ?>checked="checked"<?php endif;?> value="1">
										</div>
									</td>
									<td class="text-center">
										<div class="checkbox checkbox-success checkbox-inline">
											<input <?php echo !current_user_can('CanManageGuestBookings') ? 'disabled' : '';?> id="package_items_upgrade_<?php echo $i; ?>" name="package_items[<?php echo $i; ?>][upgrade]" type="checkbox" <?php if ($booking_item['Upgrade']) : ?>checked="checked"<?php endif;?> value="1">
										</div>
									</td>
