...
abstract class Package implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\TheFarm\\Models\\Map\\PackageTableMap';
    ...
    /**
     * Initializes internal state of TheFarm\Models\Base\Package object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\TheFarm\Models\Package The current object (for fluent API support)
     */
    ...
    public function setPackageName($v)
    {
        ...
        if ($this->package_name !== $v) {
            $this->package_name = $v;
            $this->modifiedColumns[PackageTableMap::COL_PACKAGE_NAME] = true;
        }
    ...
    public function setPackageType($v)
    {
        ...
        if ($this->package_type !== $v) {
            $this->package_type = $v;
            $this->modifiedColumns[PackageTableMap::COL_PACKAGE_TYPE] = true;
        }
    ...
    public function setDuration($v)
    {
        ...
        if ($this->duration !== $v) {
            $this->duration = $v;
            $this->modifiedColumns[PackageTableMap::COL_DURATION] = true;
        }
    ...
    public function setPackageTypeId($v)
    {
        ...
        if ($this->package_type_id !== $v) {
            ...
            $this->modifiedColumns[PackageTableMap::COL_PACKAGE_TYPE_ID] = true;
        }
    ...
    public function setPersonalized($v)
    {
        ...
        if ($this->personalized !== $v) {
            $this->personalized = $v;
            $this->modifiedColumns[PackageTableMap::COL_PERSONALIZED] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PackageTableMap::translateFieldName('PackageName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->package_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PackageTableMap::translateFieldName('PackageType', TableMap::TYPE_PHPNAME, $indexType)];
            $this->package_type = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PackageTableMap::translateFieldName('Duration', TableMap::TYPE_PHPNAME, $indexType)];
            $this->duration = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : PackageTableMap::translateFieldName('PackageTypeId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->package_type_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : PackageTableMap::translateFieldName('Personalized', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\TheFarm\\Models\\Package'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null !== $this->package_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . PackageTableMap::COL_PACKAGE_ID . ')');
        }
        ...
        if ($this->isColumnModified(PackageTableMap::COL_PACKAGE_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'package_name';
        }
        if ($this->isColumnModified(PackageTableMap::COL_PACKAGE_TYPE)) {
            $modifiedColumns[':p' . $index++]  = 'package_type';
        }
        if ($this->isColumnModified(PackageTableMap::COL_DURATION)) {
            $modifiedColumns[':p' . $index++]  = 'duration';
        }
        if ($this->isColumnModified(PackageTableMap::COL_PACKAGE_TYPE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'package_type_id';
        }
        if ($this->isColumnModified(PackageTableMap::COL_PERSONALIZED)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\TheFarm\Models\Package
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\TheFarm\Models\Package The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PackageTableMap::COL_PACKAGE_NAME)) {
            $criteria->add(PackageTableMap::COL_PACKAGE_NAME, $this->package_name);
        }
        if ($this->isColumnModified(PackageTableMap::COL_PACKAGE_TYPE)) {
            $criteria->add(PackageTableMap::COL_PACKAGE_TYPE, $this->package_type);
        }
        if ($this->isColumnModified(PackageTableMap::COL_DURATION)) {
            $criteria->add(PackageTableMap::COL_DURATION, $this->duration);
        }
        if ($this->isColumnModified(PackageTableMap::COL_PACKAGE_TYPE_ID)) {
            $criteria->add(PackageTableMap::COL_PACKAGE_TYPE_ID, $this->package_type_id);
        }
        if ($this->isColumnModified(PackageTableMap::COL_PERSONALIZED)) {
            $criteria->add(PackageTableMap::COL_PERSONALIZED, $this->personalized);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \TheFarm\Models\Package (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \TheFarm\Models\Package Clone of current object.
     * @throws PropelException
     */
    ...
    public function setBookings(Collection $bookings, ConnectionInterface $con = null)
    {
        ...
        foreach ($bookingsToDelete as $bookingRemoved) {
            $bookingRemoved->setPackage(null);
        }
    ...
    public function setPackageItems(Collection $packageItems, ConnectionInterface $con = null)
    {
        ...
        foreach ($packageItemsToDelete as $packageItemRemoved) {
            $packageItemRemoved->setPackage(null);
        }
     ...
     * @return ObjectCollection|ChildPackageItem[] List of ChildPackageItem objects
     */
    public function getPackageItemsJoinItem(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildPackageItemQuery::create(null, $criteria);
        $query->joinWith('Item', $joinBehavior);
