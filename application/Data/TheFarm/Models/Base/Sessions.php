...
/**
 * Base class that represents a row from the 'tf_sessions' table.
 *
 *
 *
...
abstract class Sessions implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\TheFarm\\Models\\Map\\SessionsTableMap';
    ...
    /**
     * Initializes internal state of TheFarm\Models\Base\Sessions object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\TheFarm\Models\Sessions The current object (for fluent API support)
     */
    ...
    public function setTimestamp($v)
    {
        ...
        if ($this->timestamp !== $v) {
            $this->timestamp = $v;
            $this->modifiedColumns[SessionsTableMap::COL_TIMESTAMP] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SessionsTableMap::translateFieldName('IpAddress', TableMap::TYPE_PHPNAME, $indexType)];
            $this->ip_address = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SessionsTableMap::translateFieldName('Timestamp', TableMap::TYPE_PHPNAME, $indexType)];
            $this->timestamp = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : SessionsTableMap::translateFieldName('Data', TableMap::TYPE_PHPNAME, $indexType)];
            if (null !== $col) {
                $this->data = fopen('php://memory', 'r+');
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\TheFarm\\Models\\Sessions'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SessionsTableMap::COL_TIMESTAMP)) {
        ...
        $sql = sprintf(
            'INSERT INTO tf_sessions (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\TheFarm\Models\Sessions
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\TheFarm\Models\Sessions The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SessionsTableMap::COL_TIMESTAMP)) {
            $criteria->add(SessionsTableMap::COL_TIMESTAMP, $this->timestamp);
        }
        if ($this->isColumnModified(SessionsTableMap::COL_DATA)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \TheFarm\Models\Sessions (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \TheFarm\Models\Sessions Clone of current object.
     * @throws PropelException
     */
