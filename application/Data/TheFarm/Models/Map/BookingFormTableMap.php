...
use TheFarm\Models\BookingForm;
use TheFarm\Models\BookingFormQuery;
 ...
 *
 */
class BookingFormTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'TheFarm.Models.Map.BookingFormTableMap';
     ...
     */
    const OM_CLASS = '\\TheFarm\\Models\\BookingForm';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'TheFarm.Models.BookingForm';
  ...
  array (
    ...
    {
        return $withPrefix ? BookingFormTableMap::CLASS_DEFAULT : BookingFormTableMap::OM_CLASS;
    }
     ...
     * @return array           (BookingForm object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = BookingFormTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = BookingFormTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + BookingFormTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = BookingFormTableMap::OM_CLASS;
            /** @var BookingForm $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            BookingFormTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = BookingFormTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = BookingFormTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(BookingFormTableMap::DATABASE_NAME)->getTable(BookingFormTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(BookingFormTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(BookingFormTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new BookingFormTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a BookingForm or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or BookingForm object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \TheFarm\Models\BookingForm) { // it's a model object
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ((array) $values as $singleval) {
                BookingFormTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return BookingFormQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a BookingForm or Criteria object.
     *
     * @param mixed               $criteria Criteria or BookingForm object containing data that is used to create the INSERT statement.
...
//
BookingFormTableMap::buildTableMap();
