...
use TheFarm\Models\PackageItems;
use TheFarm\Models\PackageItemsQuery;
 ...
 *
 */
class PackageItemsTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'TheFarm.Models.Map.PackageItemsTableMap';
     ...
     */
    const OM_CLASS = '\\TheFarm\\Models\\PackageItems';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'TheFarm.Models.PackageItems';
    ...
    protected static $fieldNames = array (
        ...
        self::TYPE_COLNAME       => array(PackageItemsTableMap::COL_PACKAGE_ID, PackageItemsTableMap::COL_ITEM_ID, PackageItemsTableMap::COL_QUANTITY, ),
    ...
    protected static $fieldKeys = array (
        ...
        self::TYPE_COLNAME       => array(PackageItemsTableMap::COL_PACKAGE_ID => 0, PackageItemsTableMap::COL_ITEM_ID => 1, PackageItemsTableMap::COL_QUANTITY => 2, ),
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('PackageItems');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\TheFarm\\Models\\PackageItems');
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \TheFarm\Models\PackageItems $obj A \TheFarm\Models\PackageItems object.
     ...
     *
     * @param mixed $value A \TheFarm\Models\PackageItems object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \TheFarm\Models\PackageItems) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \TheFarm\Models\PackageItems object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? PackageItemsTableMap::CLASS_DEFAULT : PackageItemsTableMap::OM_CLASS;
    }
     ...
     * @return array           (PackageItems object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = PackageItemsTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = PackageItemsTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + PackageItemsTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = PackageItemsTableMap::OM_CLASS;
            /** @var PackageItems $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            PackageItemsTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = PackageItemsTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = PackageItemsTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(PackageItemsTableMap::COL_PACKAGE_ID);
            $criteria->addSelectColumn(PackageItemsTableMap::COL_ITEM_ID);
            $criteria->addSelectColumn(PackageItemsTableMap::COL_QUANTITY);
        } else {
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(PackageItemsTableMap::DATABASE_NAME)->getTable(PackageItemsTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(PackageItemsTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(PackageItemsTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new PackageItemsTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a PackageItems or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or PackageItems object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \TheFarm\Models\PackageItems) { // it's a model object
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(PackageItemsTableMap::COL_PACKAGE_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(PackageItemsTableMap::COL_ITEM_ID, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = PackageItemsQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            PackageItemsTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                PackageItemsTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return PackageItemsQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a PackageItems or Criteria object.
     *
     * @param mixed               $criteria Criteria or PackageItems object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from PackageItems object
        }
...
} // PackageItemsTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
PackageItemsTableMap::buildTableMap();
