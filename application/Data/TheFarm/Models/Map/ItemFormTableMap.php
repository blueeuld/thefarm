...
use TheFarm\Models\ItemForm;
use TheFarm\Models\ItemFormQuery;
 ...
 *
 */
class ItemFormTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'TheFarm.Models.Map.ItemFormTableMap';
     ...
     */
    const OM_CLASS = '\\TheFarm\\Models\\ItemForm';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'TheFarm.Models.ItemForm';
    ...
    protected static $fieldNames = array (
        ...
        self::TYPE_COLNAME       => array(ItemFormTableMap::COL_FORM_ID, ItemFormTableMap::COL_ITEM_ID, ),
    ...
    protected static $fieldKeys = array (
        ...
        self::TYPE_COLNAME       => array(ItemFormTableMap::COL_FORM_ID => 0, ItemFormTableMap::COL_ITEM_ID => 1, ),
    ...
    public function initialize()
    {
        ...
        $this->setPhpName('ItemForm');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\TheFarm\\Models\\ItemForm');
        $this->setPackage('TheFarm.Models');
        $this->setUseIdGenerator(false);
        // columns
        $this->addForeignPrimaryKey('form_id', 'FormId', 'INTEGER' , 'tf_forms', 'form_id', true, null, null);
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \TheFarm\Models\ItemForm $obj A \TheFarm\Models\ItemForm object.
     ...
     *
     * @param mixed $value A \TheFarm\Models\ItemForm object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \TheFarm\Models\ItemForm) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \TheFarm\Models\ItemForm object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? ItemFormTableMap::CLASS_DEFAULT : ItemFormTableMap::OM_CLASS;
    }
     ...
     * @return array           (ItemForm object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = ItemFormTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = ItemFormTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + ItemFormTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = ItemFormTableMap::OM_CLASS;
            /** @var ItemForm $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            ItemFormTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = ItemFormTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = ItemFormTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(ItemFormTableMap::COL_FORM_ID);
            $criteria->addSelectColumn(ItemFormTableMap::COL_ITEM_ID);
        } else {
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(ItemFormTableMap::DATABASE_NAME)->getTable(ItemFormTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(ItemFormTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(ItemFormTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new ItemFormTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a ItemForm or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or ItemForm object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \TheFarm\Models\ItemForm) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(ItemFormTableMap::DATABASE_NAME);
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(ItemFormTableMap::COL_FORM_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(ItemFormTableMap::COL_ITEM_ID, $value[1]));
                $criteria->addOr($criterion);
            }
        }

        $query = ItemFormQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            ItemFormTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                ItemFormTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return ItemFormQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a ItemForm or Criteria object.
     *
     * @param mixed               $criteria Criteria or ItemForm object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from ItemForm object
        }
...
//
ItemFormTableMap::buildTableMap();
